...
public class NewTestMemoryAllocator {
	...
	public static void main(String... args) {
		...
		// assign random values for idx
		final int[] idx = new int[nElements];
		for (int i = 0; i < nElements; i++) {
			idx[i] = (int) (Math.random() * nElements);
		}

		ObjectType t = Allocator.allocate(
				Allocator.Type.valueOf(args[0]), nElements);
		...
		for (int x = 0; x < 100; x++) {
				...
				System.out
						.println(String
								.format("[%2s] %s - [Write %16s ms, Read %16s ms, Rand Read %16s ms], "
										...
										readMs,
										rreadMs,
										(TimeUnit.SECONDS.toNanos(nElements) / totalWrite)
												/ ONE_MILLION,
										(TimeUnit.SECONDS.toNanos(nElements) / totalRead)
												/ ONE_MILLION,
										(TimeUnit.SECONDS.toNanos(nElements) / rtotalRead)
												/ ONE_MILLION, resWrite,
										resRead, rresRead));
			}
	}
